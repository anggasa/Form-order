(function(){
    ...
    for (i=0; i<len; i++){
        (function(script){
            ...
            if (!target123Domain || isNaN(formId) || formId < 1 || ( [ 'text/javascript', 'text/rocketscript' ].indexOf(type) < 0 ) || script.getAttribute('data-role') == null || script.getAttribute('data-role') != 'form') {
                return;
            }
            ...
            iframe.src = (
                iframeSrc = target123Domain
                    ...
                    + 'ref=' + encodeURIComponent(windowTop.replace(/(<([^>]+)>)/ig, '')).replace('%26', '[%ANDCHAR%]')
            ...
            (Resizer||iFrameResize)({ heightCalculationMethod: (navigator.userAgent.indexOf("MSIE") !== -1) ? 'max' : 'documentElementOffset', checkOrigin: false}, iframe);
